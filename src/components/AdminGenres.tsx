...
import { 
  PlusCircle, Edit, Trash2, Search, X, Tag, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export default function AdminGenres({
  ...
  const [currentGenreToEdit, setCurrentGenreToEdit] = useState<Genre | null>(null);
  ...
  const paginatedGenres = useMemo(() => {
    ...
    return filteredGenres.slice(startIndex, startIndex + itemsPerPage);
  ...
  const handleOpenEditModal = (g: Genre) => {
    setCurrentGenreToEdit(g);
    setFormName(g.name);
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    const nextId = genres.reduce((max, g) => g.id > max ? g.id : max, 0) + 1;
  ...
  const handleDeleteClick = (id: number, name: string) => {
    if (confirm(`¿Estás seguro de eliminar el género musical "${name}"? Esto afectará la clasificación de canciones en la tienda.`)) {
      onDeleteGenre(id);
    }
  };
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Clasificación de Géneros</h2>
          <p className="text-xs text-gray-500 mt-1">
            Administración del catálogo de categorizaciones y etiquetas de sonido en la base de datos Chinook.
          </p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm select-none">
        ...
        <span className="text-xs text-gray-500 font-medium">Total: <strong>{filteredGenres.length}</strong> géneros activos</span>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 font-semibold text-[10.5px] uppercase tracking-wider select-none">
                ...
                <th className="py-3 px-6">Etiqueta de Género</th>
                <th className="py-3 px-6 text-center w-40">Identificador Interno</th>
              ...
              {paginatedGenres.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-gray-400 font-medium">
                    No se encontraron géneros musicales registrados con ese criterio.
                  </td>
                </tr>
              ) : (
                paginatedGenres.map((g) => (
                  <tr key={g.id} className="hover:bg-gray-50 transition">
                    <td className="py-3 px-6 text-center font-mono font-medium text-gray-400">#GEN-0{g.id}</td>
                    <td className="py-4 px-6 font-bold text-gray-900 group-hover:text-blue-600 transition text-sm">
                      <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 font-semibold border border-blue-100 text-xs">
                        {g.name}
                      </span>
                    </td>
                    <td className="py-3 px-6 text-center text-gray-500 font-mono font-bold text-xs">{g.id}</td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-4 border-b border-gray-100 pb-3 text-left">
              ...
              <h3 className="font-bold text-lg text-gray-900 font-sans">Registrar Nuevo Género</h3>
            </div>

            <form onSubmit={handleAddSubmit} className="space-y-4 text-left">
              <div>
                <label className="block text-xs text-gray-500 font-semibold mb-1">Nombre de Género *</label>
                <input
                  type="text"
                  placeholder="Ej. Reggae"
                  value={formName}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-4 border-b border-gray-100 pb-3 text-left">
              ...
              <h3 className="font-bold text-lg text-gray-900 font-sans">Editar Género</h3>
            </div>

            <form onSubmit={handleEditSubmit} className="space-y-4 text-left">
              <div>
                <label className="block text-xs text-gray-500 font-semibold mb-1">Nombre del Género *</label>
                <input
                  type="text"
                  value={formName}
                  onChange={(e) => setFormName(e.target.value)}
